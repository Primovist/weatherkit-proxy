...
test("airQualityScale: 含点的无版本 alias 不被误当成版本号", async () => {
    let called = false;
    const restore = stubFetch(() => {
        called = true;
        return { status: 404, body: "Not Found" };
    ...
    try {
        const res = await Response({ url: `${APPLE}/zh-Hans-CN/EU.EAQI`, method: "GET", headers: {} }, { status: 404, headers: { "content-type": "text/plain" }, body: "Not Found" }, {});
        assert.equal(res.status, 200);
        assert.equal(JSON.parse(res.body).name, "EU.EAQI");
        assert.equal(called, false, "EU.EAQI 无数字版本，不应改写为 EU.2604");
    } finally {
        restore();
    }
});
